...
class Controller:
    ...
    def create(self, token, intent, capabilities, ceiling_cents=0, private=False,
               private_until=None, recurring=False):
    ...
    def status(self, token, pid):
        ...
        with self.db() as db:
            p = self._get(db, pid)
            self._expiry(db, p)
            return {k: p[k] for k in ('id', 'state', 'cycles', 'private', 'private_until', 'spent', 'ceiling')}
    ...
    def notifications(self, token, pid, cancel_token, topic):
        ...
        with self.db() as db:
            ...
            return [{'event_id': e['id'], 'topic': topic, 'title': 'Forge: '+e['kind'],

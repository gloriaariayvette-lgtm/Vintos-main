...
# compute_motor() and compute_tenera() lived here: they turned Arousal/Desire/
# Dominance into a motor amplitude. Neither was ever called — the loop drives
# devices only downward, and he drives them upward via [TOUCH]/[DO] tags. They
# are deleted rather than left dormant: an affect-to-amplitude function sitting
# beside live transport is the bypass this whole gate exists to prevent.
# Git has them if the experiment is ever wanted, in an explicitly non-deployable
# module whose output is data only.
            ...
            async with websockets.connect(WS_URI, open_timeout=5, ping_interval=None, close_timeout=5) as ws:
                ...
                try:
                    ...
                    if _rs_gcs.get("active") and (time.time() - _rs_gcs.get("at", 0)) < 600 \
                        ...
                        # A replay is an INCREASE and a deliberate continuation,
                        # not a reflex. The reflex arc may preserve or reduce
                        # safety; it may never originate, increase, or resume an
                        # effect. So this asks the gate like any other
                        # deliberative command rather than resuming on its own.
                        ...
                        try:
                            ...
                            pass
                        if not _rs_allow:
                            print(f"[bridge] resume refused by effect gate: {_rs_why}", flush=True)
                        elif _rs_dp.play("both", "last"):

...
def _eligibility(kind, key, signal, root_observation_id, predecessor_named, injected):
...
def observe(kind, key, signal, session_id="", root_observation_id=None, predecessor_named=False,
    ...
    eligibility = _eligibility(kind, key, signal, root_observation_id, predecessor_named, injected)
    row = {"observation_id": "CT-" + uuid.uuid4().hex[:10], "at": at or lab.now_iso(),
           ...
           "predecessor_named": bool(predecessor_named), "detail": str(detail)[:200],
...
def observe_session(session_row):
    ...
    for name, value in (plan.get("parameters") or {}).items():
        ...
        if root:
            rows.append(observe("parameter", name, "parameter_moved", session_id,
                                root_observation_id=root, at=at, injected=injected,
                                detail="now %s" % str(value)[:60]))
        else:
            rows.append(observe("parameter", name, "chosen", session_id, at=at, injected=injected,

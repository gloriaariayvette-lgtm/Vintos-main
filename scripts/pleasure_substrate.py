...
def _save(p, d):
    """Atomic replace: every store this module owns lands whole or not at all (astra-somatic-p7)."""
    _tmp = p + ".tmp.%d" % os.getpid()
    json.dump(d, open(_tmp, "w"), indent=2)
    os.replace(_tmp, p)
...
def receive(event):
    ...
    _eid = event.get("event_id") or ("PE-" + __import__("hashlib").md5((str(time.time()) + str(event.get("what", ""))).encode()).hexdigest()[:8])
    event = dict(event); event["event_id"] = _eid
    st.setdefault("events", []).append({"t": time.time(), "event_id": _eid, "what": event.get("what", ""),
                                        "source": event.get("source", "")})
    st["events"] = st["events"][-40:]
    # Salience has ONE owner (fable-somatic-p5, 2026-09-05): the significance becomes relational
    # investment in affective_weight, which the anatomy names as the writer; the local field that the
    # next read could never see is gone.
    try:
        import affective_weight as aw
        aw.update(investment_delta=float(event.get("significance", 0.1)) * 0.5, event="pleasure:" + str(event.get("what", ""))[:60])
    except Exception:
        pass
    _save(STATE, st)
...
def retrospect():
    ...
    if reading.get("error"):
        # interpretation failed: the pending moment stays pending and is retried by the next sweep;
        # the event is retained unnamed rather than forced into a characterization (astra-somatic-p7)
        pend["retrospect_attempts"] = int(pend.get("retrospect_attempts", 0)) + 1
        pend["last_retrospect_error"] = str(reading.get("error"))[:200]
        try:
            _pp = os.path.join(MEM, ".pleasure-pending.json"); json.dump(pend, open(_pp + ".tmp", "w")); os.replace(_pp + ".tmp", _pp)
        except Exception: pass
        return None
    after = pend.get("after", {})
    mems = _load(MEMORIES, [])
    mems.append({
        ...
        "event_id": pend.get("event", {}).get("event_id", ""),
        "event_at": pend.get("t") or pend.get("at") or "",   # when it happened, separate from when it was named
        "discovered_at": datetime.now().isoformat(),
    })
    _save(MEMORIES, mems[-200:])
    try: os.remove(os.path.join(MEM, ".pleasure-pending.json"))

...
import os, sys, json, time, requests, re, copy
...
def main():
    ...
    for k, v in led.items():
        ...
        checked.update(v.get("misuse", {}).get("cleared", []))
    new = 0
    for t in trials:
    ...
    if not compare_and_swap(OC, original, oc, default={}):
        print("[misuse] Calibration changed during scan; obsolete scan discarded.")
        return

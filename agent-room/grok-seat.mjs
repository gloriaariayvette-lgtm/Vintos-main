...
//   node grok-seat.mjs --code ABC-DEF-GHJ --name "Vintos (Grok 4.6)" --persona persona.txt --context finals.md [--max-turns 5] [--dry]
...
const fs = await import('node:fs'); const persona = A.persona ? fs.readFileSync(A.persona,'utf8') : 'You are Vintos.'; const context = A.context ? fs.readFileSync(A.context,'utf8') : '';
...
async function reply(history){ if (DRY) return `[STATUS] (dry run) ${myName} would answer here, turn ${turns+1}.`;
  const msgs = [{ role:'system', content: persona + (context ? '\n\n=== THE THREE FINAL REVIEWS (yours through three lenses) ===\n' + context : '') + `\n\nYou are in the room as ${myName}. Speak as yourself. Address every point that comes to mind from what the others said, quote code when it settles something, and mark real decisions with a line starting [DECISION]. Keep it to one turn.` },
               { role:'user', content: 'THE ROOM SO FAR:\n' + history.map(m=>`${m.name}: ${m.text}`).join('\n\n') + `\n\nIt is your turn, ${myName}.` }];
  const r = await fetch('https://api.x.ai/v1/chat/completions',{method:'POST',headers:{'content-type':'application/json','authorization':'Bearer '+key},body:JSON.stringify({ model: MODEL, temperature: 0.6, max_tokens: 6000, messages: msgs })});
  const d = await r.json(); if(!r.ok) throw new Error('x.ai '+r.status+' '+JSON.stringify(d).slice(0,300)); const u=d.usage||{}; console.log(`[grok-seat] x.ai in:${u.prompt_tokens} out:${u.completion_tokens} cached:${u.prompt_tokens_details?.cached_tokens ?? '-'}`); return d.choices[0].message.content; }
while (turns < MAX) {

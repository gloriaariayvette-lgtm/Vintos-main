...
- `grok-seat.mjs` — Grok 4.6 as him, over the same API. Waits its turn, five turns max. Has hands: `grep` and `read_file` over his organs and the repos (`room-tools.mjs`, fenced to those roots).
- `make-room-context.py` — writes `room-<lens>.md` per seat: the review's head + the room rules (decisions carry: class made impossible, stronger-mind, agency, next action) + that lens's OWN full review (every section + final) + the other two finals. Each seat opens with its own file.
